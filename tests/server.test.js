...
const CUSTOM_SCENARIOS_PATH = path.join(__dirname, "..", "data", "custom-scenarios.json");
...
function removeCustomScenario(id) {
  if (!id) return;
  if (!fs.existsSync(CUSTOM_SCENARIOS_PATH)) return;

  const scenarios = JSON.parse(fs.readFileSync(CUSTOM_SCENARIOS_PATH, "utf8"));
  delete scenarios[id];
  if (Object.keys(scenarios).length === 0) {
    fs.rmSync(CUSTOM_SCENARIOS_PATH);
  } else {
    fs.writeFileSync(CUSTOM_SCENARIOS_PATH, `${JSON.stringify(scenarios, null, 2)}\n`);
  }
}

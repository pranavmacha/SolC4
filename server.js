  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (requestUrl.pathname.startsWith("/api/")) {
        await handleApi(req, res, requestUrl.pathname, { authRequired });
        return;
      }
...
async function handleApi(req, res, pathname, options) {
  ...
  switch (key) {
    ...
    case "GET /api/scenarios":
      ...
      return sendJson(res, 200, { scenarios: scenarioStore.listScenarios() });
    ...
    case "POST /api/scenarios":
      ...
      return await handleCreateScenario(req, res);
...
async function handleAssistant(req, res) {
  const payload = await readJson(req);
  sendJson(res, 200, await generateAssistantResponse(payload));
}

async function handleBriefing(req, res) {
  const payload = await readJson(req);
  sendJson(res, 200, await generateOperationsBriefing(payload));
}
...
function verifySessionCookie(value) {
  ...
  if (!payload || !signature || signSessionPayload(payload) !== signature) return false;
